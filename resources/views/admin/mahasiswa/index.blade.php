...
@section('content')
    <div class="space-y-6">
        ...
        <!-- Bulk Actions -->
        <form id="bulk-form" action="{{ route('admin.mahasiswa.bulk-delete') }}" method="POST">
            @csrf
            @method('DELETE')
            
            <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-4 flex items-center justify-between">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($mahasiswas as $mahasiswa)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <input type="checkbox" name="ids[]" value="{{ $mahasiswa->id }}" class="row-checkbox h-4 w-4 text-primary-600 border-gray-300 rounded">
                                </td>
